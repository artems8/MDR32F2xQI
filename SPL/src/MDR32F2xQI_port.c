...
void PORT_Init(MDR_PORT_TypeDef* PORTx, const PORT_InitTypeDef* PORT_InitStruct)
{
    ...
    PORTx->OE     = tmpreg_OE & (~SWD_PINS(PORTx));
    PORTx->FUNC   = tmpreg_FUNC & (~SWD_PINS(PORTx));
    PORTx->ANALOG = tmpreg_ANALOG & (~SWD_PINS(PORTx));
    PORTx->PULL   = tmpreg_PULL & (~SWD_PINS(PORTx));
    PORTx->PD     = tmpreg_PD & (~SWD_PINS(PORTx));
    PORTx->PWR    = tmpreg_PWR & (~SWD_PINS(PORTx));
    PORTx->GFEN   = tmpreg_GFEN & (~SWD_PINS(PORTx));
}
