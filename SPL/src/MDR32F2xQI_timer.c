...
ITStatus TIMER_GetITStatus(MDR_TIMER_TypeDef* TIMERx, TIMER_Status_Flags_TypeDef TIMER_IT)
{
    ...
    tmpreg = TIMERx->STATUS & TIMERx->IE & (uint32_t)TIMER_IT;
...
/**
  * @brief  Initializes the TIMERx peripheral Clock according to the specified parameters.
  * @param  TIMERx: select the TIMER peripheral.
  *         This parameter can be one of the following values: MDR_TIMER0, MDR_TIMER1.
...
void TIMER_BRGInit(MDR_TIMER_TypeDef* TIMERx, TIMER_Clock_BRG_TypeDef TIMER_BRG)
{
    ...
    if (TIMERx == MDR_TIMER0)
    {
        tmpreg &= ~RST_CLK_TIM_CLOCK_TIM0_BRG_Msk;
        tmpreg |= TIMER_BRG << RST_CLK_TIM_CLOCK_TIM0_BRG_Pos;
        tmpreg |= RST_CLK_TIM_CLOCK_TIM0_CLK_EN;
    }
    else if (TIMERx == MDR_TIMER1)
    {
