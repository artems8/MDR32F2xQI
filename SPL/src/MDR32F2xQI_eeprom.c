...
__RAMFUNC void ProgramDelayAsm(uint32_t Loops) __attribute__((section("EXECUTABLE_MEMORY_SECTION")));
...
__RAMFUNC void EEPROM_ErasePage(uint32_t Address, EEPROM_Mem_Bank BankSelector)
{
    ...
    MDR_EEPROM->CMD = Command;
    ProgramDelayAsm(GET_US_LOOPS(10));     /* Wait for 10 us */
    Command |= EEPROM_CMD_NVSTR;
    MDR_EEPROM->CMD = Command;
    ProgramDelayAsm(GET_US_LOOPS(30000));  /* Wait for 30 ms */
    Command &= ~EEPROM_CMD_ERASE;
    MDR_EEPROM->CMD = Command;
    ProgramDelayAsm(GET_US_LOOPS(5));      /* Wait for 5 us */
    Command &= ~(EEPROM_CMD_XE | EEPROM_CMD_NVSTR);
    MDR_EEPROM->CMD = Command;
    ProgramDelayAsm(GET_US_LOOPS(1));      /* Wait for 1 us */

    Command &= EEPROM_CMD_DELAY_Msk;
    MDR_EEPROM->CMD = Command;
    MDR_EEPROM->KEY = 0;
    
    ProgramDelayAsm(GET_US_LOOPS(4000));             /* Wait for 4 ms */
}
...
__RAMFUNC void EEPROM_EraseBlock(uint32_t Address, EEPROM_Mem_Bank BankSelector)
{
    ...
    Command &= ~EEPROM_CMD_ERASE;
    MDR_EEPROM->CMD = Command;
    ProgramDelayAsm(GET_US_LOOPS(100));              /* Wait for 100 us */
...
__RAMFUNC void EEPROM_ProgramWord(uint32_t Address, EEPROM_Mem_Bank BankSelector, uint32_t Data)
{
    ...
    MDR_EEPROM->CMD = Command;
    ProgramDelayAsm(GET_US_LOOPS(30));               /* Wait for 30 us */
    Command &= ~EEPROM_CMD_YE;
    MDR_EEPROM->CMD = Command;
